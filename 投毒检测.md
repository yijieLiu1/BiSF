> [!NOTE]
>
> 代码运行时间问题。以下实验均只跑了20轮，映射为1024维，粗略估计投毒检测的状况。
...
### 	30%投毒，0.3rate，3*3Trigger（也测试了2X2的Trigger，效果类似）

​	1.原始模型参数：[clustering]最优，f1=18.95%，exact_match=15.00%，当投毒达到30%时，原始向量的检测均会急剧下跌。特别是Multi+Geo

​	2.原始模型参数正交映射：[clustering]最优， f1=53.33%，exact_match=45.00%。Cluster后几轮的检测效果很好**，但是很奇怪，为什么正交后反而更准**

​	3.简化提取的原始模型参数：[clustering]最优，f1=74.07%，exact_match=65.00%，Multi+Geo全崩。全是0

​	4.简化+正交的模型参数：[clustering]最优，f1=82.14%，exact_match=75.00%，Multi+Geo全崩。全是0



# 小结	

​	【1】正交映射后的影响不大。映射到1024和2048效果无太大差别，在某些情况下，甚至会出现正交映射后的检测效果更好。

​	【2】定向翻转和非定向的整体趋势类似，非定向的检测效果会更好一些。

​	【3】检测方法：

​		1.投毒量少（10%，20%），参数未精简提取的时候。Cluster表现很差，Multi和Geo表现很好。参数精简提取后，Cluster效果会显著提升，成为最优。

​		2.投毒30%，参数未精简提取的时候。Multi和Geo表现非常差，Cluster表现好，但也就30%左右。参数精简提取后，Cluster效果会显著提升，但是Multi和Geo没有太大变化。

​	**参数精简对于cluster非常有效，cluster在恶意DO多的情况下，效果普遍更好。Multi和Geo能检测到这两种标签投毒，且在恶意DO少，全量参数的情况下，甚至更优**



但是目前的方案，有几个问题：1.轮数有点少，效果普遍是刚开始差一些，后面检测出来后，就一直很不错。但之前100轮的时候，发现效果先好后差的情况

​							2.客户端的数量还是不够多，才10个DO。

后续敲定具体实验配置，可以跑一些多轮，多DO的。