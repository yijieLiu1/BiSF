...
## 正确性实验（无安全向量内积，无Paillier加密）

### 1.无任何攻击投毒，正常的训练，直至收敛。

​	CNN+MNIST ：50轮，10个DO，学习率0.0001

​	resnet20+cifar10：150轮，10个DO，学习率0.0005

### 2.投毒

​	配置同上，攻击的比例依次10%，20%，30%，40%。	
...
​	2.4 lie-stat （统计型放大）**？有点未知**

​	2.5 Labelflip（选定一个标签，随机翻转成已有的某个标签）

​	2.6 backdoor（2*2像素块）

### 3.投毒＋检测筛除

​	【正交参数的映射：512，1024，2048】

​	检测方法配套三种：multi_krum；geomedian；clustering。

​	如果只选择其中某一个检测，然后剔除。会导致实验量爆增，**216项实验（24X*3*X3）。**考虑缩减比例，只考虑20%和40%投毒的比例？变成108项实验。

> [!NOTE]
>
> CNN共计需要，1+4*6，25个实验，明文下，**单个实验耗时：25min** 
>
> resnet20，单个实验耗时：250min

​	**1.记录有模型训练的精度Acc，可以进行精度Acc的比较。（1）无投毒（2）投毒（3）投毒＋检测**
​	**2.labelflip和backdoor的Acc降低不明显，有ASR指标。**
...
> [!CAUTION]
>
> 核心的关键点在于统计，增设了安全向量内积和加密方案后，单个DO下的计算耗时。

### 1.训练用时

2项实验，CNN+MNIST；resnet20+cifar10
...
SIP-r1=90s  SIP-r2=30s  SIP-r3=0.002s

CSP聚合解密：74s

一轮：6x10+30x10+74=434s

单个DO：加密6s+安全向量内积30s+CSP聚合解密74s=110s
...
SIP-r1=380s  SIP-r2=144s  SIP-r3=0.02s

CSP聚合解密：367s

一轮：27x10+144x10+367=2077s（34min）

单个DO：加密27s+安全向量内积144s+CSP聚合解密367s=540s（9min）
...
【0-1024】SIP-r1=400s  SIP-r2=150s  SIP-r3=0.02s

【1024-2048】SIP-r1=400s  SIP-r2=150s  SIP-r3=0.02s

CSP聚合解密：367s

一轮：27x10+144x2x10+367=3517（59min）

单个DO：加密27s+安全向量内积288s+CSP聚合解密367s=682s（11min）
